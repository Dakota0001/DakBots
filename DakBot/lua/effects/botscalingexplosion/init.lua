function EFFECT:Init( data )
	...
	if dustsize > 1 then
		...
		if groundtrace.Hit then
			for i = 1,dustsize*2 do
				...
				local particle = emitter:Add( "particles/smokey", groundtrace.HitPos+Vector(math.Rand(-dustsize*10,dustsize*10),math.Rand(-dustsize*10,dustsize*10),-25)) 
				 
				if particle == nil then particle = emitter:Add( "particles/smokey", groundtrace.HitPos+Vector(math.Rand(-dustsize*10,dustsize*10),math.Rand(-dustsize*10,dustsize*10),-25))  end
	...
	for i = 1,30*size do
		...
		local particle = emitter:Add( "particles/smokey", Pos + size*Vector( math.random(-10,10),math.random(-10,10),math.random(-10,10) ) ) 
		 
		if particle == nil then particle = emitter:Add( "particles/smokey", Pos + size*Vector(   math.random(0,0),math.random(0,0),math.random(0,0) ) ) end
